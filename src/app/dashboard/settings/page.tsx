...
export default function SettingsPage() {
  ...
  } = useSettingsFirestore();
  const [activeTab, setActiveTab] = useState<SettingsTab>("shop");
  const [hasChanges, setHasChanges] = useState(false);
  const [isSaving, setIsSaving] = useState(false);
  const [mobileExpanded, setMobileExpanded] = useState<SettingsTab | null>(
    "shop",
  );
  ...
  const renderPanel = (tab: SettingsTab) => {
    switch (tab) {
      ...
      case "users":
        return <UserManagementPanel locale={locale} onChange={handleChange} />;
      case "payments":
        return (
          <PaymentConfigPanel
            locale={locale}
      ...
      case "notifications":
        return <NotificationsPanel locale={locale} onChange={handleChange} />;
      case "data":
        return <DataSecurityPanel locale={locale} onChange={handleChange} />;
      case "advanced":
        return <AdvancedPanel locale={locale} onChange={handleChange} />;
      default:
        return null;
    }
  };
    ...
    <div className={isMobile ? "" : "page-contained"}>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <span className="hidden sm:inline">
                {locale === "sw" ? "Hifadhi" : "Save Changes"}
              </span>
            </Button>
          </div>
        </div>
      ...
      <div
        className={
          isMobile
            ? ""
            : "flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-[240px_1fr] gap-4"
        }
      >
        ...
        <div
          className={isMobile ? "" : "flex-1 overflow-y-auto scroll-container"}
        >
            ...
            {tabs.map((tab) => (
              ...
              >
                <button
                  onClick={() =>
                    setMobileExpanded(
                      mobileExpanded === tab.key ? null : tab.key,
                    )
                  }
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-heading font-bold text-sm text-warm-900 dark:text-warm-50">
                  ...
                  <svg
                    ...
                    stroke="currentColor"
                    strokeWidth="2"
                    className={`text-warm-400 transition-transform ${mobileExpanded === tab.key ? "rotate-180" : ""}`}
                  >
          ...
          <div className="hidden lg:block">{renderPanel(activeTab)}</div>
        </div>
      </div>
    </div>
  );

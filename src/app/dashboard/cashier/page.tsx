...
function AdminSalesInner() {
  ...
  const { sales, placeSale } = useSales();
  ...
  const dailySales = useMemo(() => {
    const todaySales = sales.filter((s) => {
      const saleDate = s.timestamp ? new Date(s.timestamp).toISOString().slice(0, 10) : "";
      return saleDate === todayStr && s.status === "completed";
    });
    return {
  ...
  useEffect(() => {
    try {
      sessionStorage.setItem("duka-admin-held", JSON.stringify(heldSales));
  ...
  useEffect(() => {
    try {
      const saved = sessionStorage.getItem("duka-admin-held");
  ...
  // Activity tracking for admin sales
  ...
  const handlePaymentComplete = useCallback(
    ...
    [grandTotal, toast, cart, selectedCustomerId, offlineState.isOnline, saveOfflineTransaction, clearPersistedCart, placeSale, firestoreCustomers, currentReceiptCode, logSale]
  );
  ...
  return (
    <div className="h-[calc(100vh-80px)] flex flex-col bg-warm-50 dark:bg-warm-900 overflow-hidden rounded-xl">
      ...
      {/* Top bar */}
      <div className="flex-shrink-0 bg-white dark:bg-warm-800 border-b border-warm-200/60 dark:border-warm-700/60 flex items-center justify-between px-3 sm:px-4 z-30" style={{ minHeight: "48px" }}>
        <div className="flex items-center gap-3">
          <h1 className="font-heading font-extrabold text-sm sm:text-base text-warm-900 dark:text-warm-50">
            ...
            <span className="text-[10px] font-medium text-warm-400 ml-2 hidden sm:inline">
              {t("Sales Portal", "Duka la Mauzo")}
            </span>
          </h1>
          {heldSales.length > 0 && (
            <button
              onClick={() => setShowHeldDrawer(true)}
              className="flex items-center gap-1 px-2 py-1 rounded-lg bg-savanna-50 dark:bg-savanna-900/15 text-savanna-600 text-[10px] font-medium min-h-[28px]"
            >
          ...
          <button
            ...
            className="flex items-center gap-1 px-2 py-1 rounded-lg bg-forest-50 dark:bg-forest-900/15 text-forest-600 text-[10px] font-medium min-h-[28px]"
          >
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          ...
          {/* Persistent scanner status - always visible */}
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="px-2 py-1 rounded-lg text-[10px] font-bold bg-warm-100 dark:bg-warm-700 text-warm-600 dark:text-warm-300 min-h-[28px] hidden sm:flex items-center gap-1"
          ...
          <button
            ...
            className="px-2 py-1 rounded-lg text-[10px] font-bold bg-warm-100 dark:bg-warm-700 text-warm-600 dark:text-warm-300 min-h-[28px]"
          >
            {locale === "en" ? "SW" : "EN"}
          </button>
          <div
          ...
          {/* Admin name + back to dashboard */}
          <div className="flex items-center gap-1.5 pl-2 border-l border-warm-200 dark:border-warm-700">
            <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-forest-500 to-savanna-500 flex items-center justify-center">
              <span className="text-white font-bold text-[9px]">
                {(profile?.displayName || profile?.email || "A").slice(0, 2).toUpperCase()}
              </span>
            </div>
            <div className="hidden sm:block min-w-0">
              <p className="text-[10px] font-bold text-warm-900 dark:text-warm-50 truncate max-w-[100px]">
                {profile?.displayName || profile?.email?.split("@")[0] || "Admin"}
              </p>
              <p className="text-[9px] text-warm-400">{t("Admin", "Msimamizi")}</p>
            </div>
            <button
              onClick={() => router.push("/dashboard")}
              title={t("Back to Dashboard", "Rudi kwenye Dashibodi")}
              className="w-7 h-7 rounded-lg flex items-center justify-center text-warm-400 hover:text-terracotta-500 hover:bg-terracotta-50 dark:hover:bg-terracotta-900/20 transition-colors ml-1"
            >

...
function CashierPortalInner() {
  const { locale, toggleLocale } = useLocale();
  const { logout, profile, shopId, currentShop } = useAuth();
  const router = useRouter();
  const toast = useToast();
  const { isLocked, reason, adminMessage } = usePortalGuard();
  const [showLockoutNotice, setShowLockoutNotice] = useState(false);
  ...
  const { sales, placeSale, dailyStats } = useCashierSales();
  ...
  useEffect(() => {
    try {
      sessionStorage.setItem("duka-cashier-held", JSON.stringify(heldSales));
  ...
  useEffect(() => {
    try {
      const saved = sessionStorage.getItem("duka-cashier-held");
  ...
  // Cashier activity tracking (writes to Firestore for profile popup)
  const {
    logLogin,
    logSale,
    logRefund,
  ...
  // Show lockout notice when portal is locked or force-logged out
  useEffect(() => {
    if (isLocked) {
      setShowLockoutNotice(true);
    }
  }, [isLocked]);

  // Track activity on cart changes
  useEffect(() => {
    if (cart.length > 0) {
      updateActivity();
    }
  ...
  // Wrap logout to log activity first
  ...
  const handlePaymentComplete = useCallback(
    async (method: string, _: number) => {
      ...
      if (offlineState.isOnline) {
        try {
          ...
          // Write activity log + update user stats for profile popup
          logSale({
            total: grandTotal,
            method,
    ...
    [grandTotal, toast, cart, selectedCustomerId, offlineState.isOnline, saveOfflineTransaction, clearPersistedCart, placeSale, firestoreCustomers, currentReceiptCode, logSale, shopId, currentShop.name]
  );

  const handleRefundComplete = useCallback(
    (refund: Record<string, unknown>) => {
      // Queue refund for sync
      enqueueSync("refund", refund, "high");

      // Log refund activity for profile popup
      logRefund({
  ...
  return (
    <div className="h-screen w-screen flex flex-col bg-warm-50 dark:bg-warm-900 overflow-hidden" style={{ height: "100dvh" }}>
      {/* Offline Banner */}
      <OfflineBanner
      ...
      {/* Top bar */}
      <div className="flex-shrink-0 bg-white dark:bg-warm-800 border-b border-warm-200/60 dark:border-warm-700/60 flex items-center justify-between px-2 sm:px-4 z-30 safe-top" style={{ paddingTop: "max(0px, env(safe-area-inset-top))", minHeight: "48px" }}>
        <div className="flex items-center gap-1.5 sm:gap-3">
          <h1 className="font-heading font-extrabold text-sm sm:text-base text-warm-900 dark:text-warm-50">
            ...
            <span className="text-[10px] font-medium text-warm-400 ml-2 hidden sm:inline">
              {t("Cashier Portal", "Duka la Mhasibu")}
            </span>
          </h1>
          {heldSales.length > 0 && (
            <button
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          ...
          <div className="flex items-center gap-1.5 pl-1.5 sm:pl-2 border-l border-warm-200 dark:border-warm-700">
            ...
            <button
              ...
              className="w-7 h-7 rounded-lg flex items-center justify-center text-warm-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
            >
      ...
      {/* Lockout / Force Logout Notice */}
      {showLockoutNotice && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-md p-4">
          <motion.div
            initial={{ opacity: 0, scale: 0.95 }}
            animate={{ opacity: 1, scale: 1 }}
            className="w-full max-w-sm rounded-2xl border border-white/20 bg-white dark:bg-warm-900 shadow-2xl p-6 text-center"
          >
            <div className={`w-16 h-16 rounded-full mx-auto mb-4 flex items-center justify-center ${
              reason === "locked" ? "bg-savanna-100 dark:bg-savanna-900/30" : "bg-red-100 dark:bg-red-900/30"
            }`}>
              <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className={
                reason === "locked" ? "text-savanna-600" : "text-red-600"
              }>
                {reason === "locked" ? (
                  <>
            ...
            <h3 className="font-heading font-bold text-lg text-warm-900 dark:text-warm-50 mb-2">
              {reason === "locked"
                ? t("Portal Suspended", "Kifagio Kimesimamishwa")
                : t("Session Ended", "Kipindi Kimeisha")}
            </h3>
            <p className="text-sm text-warm-600 dark:text-warm-400 mb-3">
              {reason === "locked"
                ? t(
                    "Your portal has been temporarily suspended by the administrator. Please contact your supervisor for assistance.",
                    "Kifagio chako kimesimamishwa kwa muda na msimamizi. Tafadhali wasiliana na msimamizi wako kwa usaidizi."
                  )
                : t(
                    "Your session has been ended by the administrator. Please log in again to continue.",
                    "Kipindi chako kimeisha na msimamizi. Tafadhali ingia tena kuendelea."
                  )}
            </p>
            {adminMessage && (
              <div className="mb-4 p-3 rounded-xl bg-terracotta-50 dark:bg-terracotta-900/20 border border-terracotta-200 dark:border-terracotta-800/30">
                <p className="text-[10px] font-bold text-terracotta-600 uppercase mb-1">
                  {t("Message from Admin", "Ujumbe kutoka Msimamizi")}
                </p>
                <p className="text-xs text-warm-700 dark:text-warm-300">{adminMessage}</p>
              </div>
            )}
            <button
              onClick={async () => {
                setShowLockoutNotice(false);
                try {
                  await logLogout();
                } catch (e) { console.warn("Operation failed:", e); }
                await logout();
              ...
              className="w-full py-3 rounded-xl bg-gradient-to-r from-terracotta-500 to-savanna-500 text-white font-heading font-bold text-sm min-h-[44px]"
            >
              {t("Go to Login Page", "Nenda kwenye Ukurasa wa Kuingia")}
            </button>
          </motion.div>
        </div>
      )}
